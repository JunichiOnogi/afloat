...
$site_top = "https://www.afloat.co.jp/";
...
$thanksPage = "./new-graduate/thanks/";
...
$require = array('姓（漢字）','名（漢字）','姓（カタカナ）','名（カタカナ）','メールアドレス','性別','生年月日（年）','生年月日（月）','生年月日（日）','年齢','携帯番号','郵便番号','都道府県','市区町村','地域・番地','専門学校名','卒業（見込み）年月','志望動機','自己PR','特技・趣味・得意科目など','第１希望店舗','第２希望店舗','InstagramのID');
...
$refrom_name = "株式会社エターナル";

// 差出人に送信確認メールを送る場合のメールのタイトル（上記で1を設定した場合のみ）
$re_subject = "採用のご応募ありがとうございました";
...
AFLOAT | Lall You | 株式会社エターナルの採用のご応募を受け付けました。
3営業日以内に担当者が確認いたします。
連絡がない場合はお手数ですがsaiyo@afloat.co.jpまでご連絡お願します。
ご応募ありがとうございました。
...
---------------------------------------------------------
AFLOAT | Lall You | 株式会社エターナル
Web Site / https://www.afloat.co.jp/
採用担当メールアドレス / saiyo@afloat.co.jp
---------------------------------------------------------
...
<div class="sec-confirm">
	   ...
	   <div class="center">
	   <p class="txt">下記の入力内容をご確認ください。<br>内容にお間違いがなければ<br class="sp">「送信する」ボタンを押してください。</p>
	   </div>
	<?php if($empty_flag == 1){ ?>
	<div class="inner error">
<div align="center">
<p>入力にエラーがあります。<br>下記をご確認の上「前画面に戻る」ボタンにて<br class="sp">修正をお願い致します。</p>
<?php echo $errm; ?><br /><br />
<button type="button" onClick="history.back()" class="btn-more center center-block">前画面に戻る</button>
</div>
</div>
<?php }else{ ?>
<div class="inner">
...
?>
	<div class="box-btn flex f-wrap f-jc-c">
                <button type="button" onClick="history.back()" class="btn-more back center center-block">入力内容を修正する</button>
                <button type="submit" class="btn-more center center-block">送信する</button>
            </div>
</form>
</div>
<?php } ?>
